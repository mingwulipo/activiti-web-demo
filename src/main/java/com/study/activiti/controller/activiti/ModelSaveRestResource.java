 ...
 */
package com.study.activiti.controller.activiti;
...
public class ModelSaveRestResource implements ModelDataJsonConstants {
  ...
  /**
   * 保存模型后，act_ge_bytearray表存入两行数据，source和source-extra，source-extra对应的是图片，但是图片的开始和结束节点的名称都没有显示
   * json_xml存入source，svg_xml存入source-extra
   * @param modelId
   * @param name
   * @param json_xml
   * @param svg_xml
  ...
  public void saveModel(@PathVariable String modelId, @RequestParam("name") String name,
    ...
    //todo 非空校验， 流程图的key和name都不能重复
    //name是模型的名称，流程图画完后，点击保存时候输入的名称，不是流程图的名称
